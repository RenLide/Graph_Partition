...
void NG::DistributeBlocks(int n, int k) {
...
//                            pre_index = pre_points[f].back().second * graph.column + pre_points[f].back().first;
    ...
    DistributeBlocks(new_num, k - kp_count[f][kp_index]);
    DistributeBlocks(n, kp_count[f][kp_index]);
}
...
pair<float, float> NG::CaculateError() {
